...
const categories = ["Escritura", "Desarrollo", "Investigación", "Análisis", "Creatividad", "General"]
...
export default function PresetForm({ initialData, onSubmit, isEditing = false }: PresetFormProps) {
  ...
  const validateForm = () => {
    ...
    const primaryModels = formData.models.filter((model) => model.role === "primary")
    ...
    if (roles.length !== uniqueRoles.size) {
      newErrors.uniqueRoles = "No puede haber modelos duplicados con el mismo rol"
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Configuración general del preset</CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Agrega y configura los modelos que formarán parte de este preset</CardDescription>
            </div>
            <Button type="button" onClick={addModel} size="sm" className="flex items-center gap-2">
        ...
        <CardContent className="space-y-4">
          ...
          {errors.primaryModel && <p className="text-sm text-red-600">{errors.primaryModel}</p>}
          {errors.uniqueRoles && <p className="text-sm text-red-600">{errors.uniqueRoles}</p>}

          {formData.models.length === 0 ? (
            <div className="text-center py-8 border-2 border-dashed border-gray-300 rounded-lg">
              <Settings className="h-12 w-12 mx-auto text-gray-400 mb-4" />
              <p className="text-gray-600 mb-4">No hay modelos configurados</p>
              ...
              {formData.models.map((model, index) => (
                <div key={index} className="relative">
                  <ModelConfigCard
                    ...
                    onUpdate={(updatedModel) => updateModel(index, updatedModel)}
      ...
      <div className="flex justify-end gap-3 pt-4">
        <Button type="submit">{isEditing ? "Actualizar Preset" : "Crear Preset"}</Button>
      </div>
    </form>
  )
}
