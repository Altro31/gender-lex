...
import { Search, Filter, RotateCcw, Trash2, Eye, Play, AlertTriangle, CheckCircle, Clock, XCircle } from "lucide-react"
...
const initialAnalyses: Analysis[] = [
  {
    ...
    duration: 300,
    inputText: "Un extenso artículo científico sobre inteligencia artificial...",
    inputSource: "file",
    results: {
      overallScore: 7.2,
      biasesDetected: [
        {
          ...
          severity: "medium",
          confidence: 0.85,
          examples: ["Los ingenieros deben...", "Las enfermeras suelen..."],
        },
        {
      ...
      biasesDetected: [
        {
          ...
          severity: "high",
          confidence: 0.92,
          examples: ["Licencia de maternidad para mujeres", "Bonos por paternidad responsable"],
        },
      ],
      totalWords: 1200,
      processedSentences: 89,
      recommendations: ["Revisar políticas de licencias parentales", "Implementar lenguaje más neutral en beneficios"],
    },
  },
  {
    id: "5",
...
export default function AnalysisPage() {
  ...
  const filteredAnalyses = analyses.filter((analysis) => {
    const matchesSearch =
      ...
      analysis.presetName.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Gestión de Análisis</h1>
          <p className="text-gray-600">Administra los análisis de detección de sesgos de género</p>
        </div>
                            ...
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                ...
                                <CardDescription className="flex items-center gap-4 text-sm">
                                  <span>Preset: {analysis.presetName}</span>
                                  <span>•</span>
                                  <span>
                                    Fuente:{" "}
                                    {analysis.inputSource === "manual"
                                      ? "Manual"
                                      : analysis.inputSource === "file"
                                        ? "Archivo"
                                        : "URL"}
                                  </span>
                                  <span>•</span>
                                ...
                                <DropdownMenuContent align="end">
                                  ...
                                  <DropdownMenuItem
                                    onClick={() => {
                                      ...
                                      console.log("Ver detalles:", analysis.id)
                                    }}
                                  >
                          ...
                          <CardContent className="pb-3">
                            <div className="space-y-4">
                              ...
                              {analysis.status === "completed" && analysis.results && (
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                  <div className="bg-gray-50 rounded-lg p-3">
                                    ...
                                    >
                                      {analysis.results.overallScore}/10
                                    </div>
                                  </div>
                                  <div className="bg-gray-50 rounded-lg p-3">
                                    <div className="text-sm text-gray-600 mb-1">Sesgos Detectados</div>
                                    <div className="text-2xl font-bold text-gray-900">
                                      {analysis.results.biasesDetected.length}
                                    </div>
                                  </div>
                                  <div className="bg-gray-50 rounded-lg p-3">
                                    <div className="text-sm text-gray-600 mb-1">Palabras Analizadas</div>
                                    <div className="text-2xl font-bold text-gray-900">
                              ...
                              {analysis.status === "completed" &&
                                analysis.results &&
                                analysis.results.biasesDetected.length > 0 && (
                                  <div className="space-y-2">
                                    <div className="text-sm font-medium text-gray-700">
                                      Categorías de Sesgo Detectadas:
                                    </div>
                                    <div className="flex flex-wrap gap-2">
                                      {analysis.results.biasesDetected.map((bias, index) => (
                                        <Badge
                                          key={index}
                                          variant="outline"
                                          className={severityConfig[bias.severity].color}
                                        >
                                          {bias.category} ({severityConfig[bias.severity].label})
                                        </Badge>
                                      ))}
                                    </div>
                                  </div>
                              ...
                              {/* Input Preview */}
                              <div className="space-y-2">
                                <div className="text-sm font-medium text-gray-700">Texto Analizado:</div>
                          ...
                          <CardFooter className="pt-3 border-t">
                            <div className="w-full flex justify-between items-center text-xs text-gray-500">
                              <div className="flex items-center gap-4">
                                ...
                                {analysis.duration && <span>Duración: {formatDuration(analysis.duration)}</span>}
                              </div>
                              {analysis.completedAt && (
                                <span>Completado: {new Date(analysis.completedAt).toLocaleString()}</span>
                              )}
                            </div>
                          </CardFooter>
                        </Card>
              ...
              <>
                {filteredAnalyses.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">No hay análisis pendientes</h3>
                    <p className="text-gray-600">Los análisis pendientes aparecerán aquí</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {filteredAnalyses.map((analysis) => {
              ...
              <>
                {filteredAnalyses.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">No hay análisis en progreso</h3>
                    <p className="text-gray-600">Los análisis en ejecución aparecerán aquí</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {filteredAnalyses.map((analysis) => {
              ...
              <>
                {filteredAnalyses.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">No hay análisis completados</h3>
                    <p className="text-gray-600">Los análisis completados aparecerán aquí</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {filteredAnalyses.map((analysis) => {
                        ...
                        <Card key={analysis.id} className="hover:shadow-md transition-shadow">
                          ...
                          <CardContent className="pb-3">
                            <div className="space-y-4">
                              ...
                              {/* Bias Categories */}
                              {analysis.results && analysis.results.biasesDetected.length > 0 && (
                                <div className="space-y-2">
                                  <div className="text-sm font-medium text-gray-700">
                                    Categorías de Sesgo Detectadas:
                                  </div>
              ...
              <>
                {filteredAnalyses.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">No hay análisis con errores</h3>
                    <p className="text-gray-600">Los análisis con errores aparecerán aquí</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {filteredAnalyses.map((analysis) => {
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                <strong className="font-medium"> {analysisToDelete?.name}</strong> y todos sus resultados.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Cancelar</AlertDialogCancel>
              <AlertDialogAction onClick={handleDeleteAnalysis} className="bg-red-600 hover:bg-red-700">
                Eliminar
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
