...
import { EditModelDialogTrigger } from "@/sections/model/components/dialogs/edit-model-dialog";
      ...
      <CardHeader>
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <CardDescription>{model.connection.identifier}</CardDescription>
          </div>
      ...
      <CardFooter className="border-t">
        <div className="w-full text-xs text-muted-foreground">
          <div className="flex justify-between">
            <span>
              {t`Created at`}:{" "}
              {new Date(model.createdAt).toLocaleDateString(i18n.locale)}
            </span>
            {model.usedAt && (
              <span>
                {t`Last use`}:{" "}
                {new Date(model.usedAt).toLocaleDateString(i18n.locale)}
              </span>
            )}
          </div>
        </div>
